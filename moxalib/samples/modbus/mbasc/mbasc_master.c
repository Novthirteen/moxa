...
/*
    ...
    Routines to simulate a simple MODBUS ASCII master.
...
send_request(MHANDLE con)
{
	...
	/* format request ADU */
	len = mbasc_packet_format(data, 6, mbasc_buf);
	/* send MODBUS ASCII request */
	if (len > 0) connection_send_data(con, (char *)mbasc_buf, len);
...
static int	
mbasc_master_dispatch (MHANDLE conp, void *private_data, DATAPKT *dpkt)
{
	...
	/* obtain a MODBUS ASCII response packet */
	len = mbasc_packet_digest(dpkt->packet_data, &plen, buf);
	if (len <= 0)
	{
...
static void*
mbasc_master_open(MHANDLE conp, void *param, unsigned int *timer_interval)
{
	...
	if (buf==NULL)
		...
		printf("mbasc_master_open: port (%d)\n", uart->port);
...
static void
mbasc_master_close(MHANDLE conp, void *private_data)
{
	(void) conp;
	/* free what was malloced at mbasc_master_open */
	if (private_data) free(private_data);
}

/* This is an initialization function where the user program opens connections */
static int
mbasc_master_init (int argc, char **argv)
{
	MHANDLE con;
	UARTPRM uart;
	USERFUN funs;
	...
	funs.open = mbasc_master_open;
	funs.dispatch = mbasc_master_dispatch;
	funs.close = mbasc_master_close;
...
#endif
{
    ...
    connection_dispatch_loop(argc, argv, mbasc_master_init, NULL, 0);
